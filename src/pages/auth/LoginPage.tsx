...
import { Mail, Lock, User, AlertCircle, Loader2, Eye, EyeOff, ArrowRight, Shield, Sparkles } from 'lucide-react';
...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.data?.error || err.data?.detail || err.data?.non_field_errors?.[0] || 'Failed to sign in';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 flex">
      ...
      {/* Right side - Form from Register Page */}
      <div className="flex-1 flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-20 xl:px-28">
        <div className="w-full max-w-md">
          <h2 className="text-2xl font-bold text-gray-800 mb-6 text-left">Welcome back</h2>
          ...
          <form className="space-y-4" onSubmit={handleSubmit} noValidate>
            ...
            >
              {isLoading ? <Loader2 className="mx-auto h-6 w-6 animate-spin" /> : 'Sign in'}
            </button>
          </form>
